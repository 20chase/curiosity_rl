...
import gym
import time
import curiosity_rl.algos.pytorch.sac.core as core
...
def sac(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    forward_model = core.ForwardModel(obs_dim[0], act_dim).to(device)
    ...
    def compute_loss_q(data):
        o, a, r, o2, d = data['obs'].to(device), data['act'].to(device), data['rew'], data['obs2'].to(device), data['done'].to(device)

        delta_pose = o2 - o

        with torch.no_grad():
            rew = forward_model.compute_loss(
                o, a, delta_pose, reduction='none', device=device
            )
        ...
        with torch.no_grad():
            ...
            backup = rew + gamma * (1 - d) * (q_pi_targ - alpha * logp_a2)
    ...
    def test_agent():
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                a = get_action(o, True)
                o2, _, d, _ = test_env.step(a)
                with torch.no_grad():
                    r = forward_model.compute_loss(
                        o, a, o2 - o, reduction='none', device=device
                    )
                o = o2
                ep_ret += r
                ep_len += 1
